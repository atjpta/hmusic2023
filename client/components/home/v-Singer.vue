    ...
    <div>
      <div class="flex justify-between mb-2">
        <span
          ...
          <span class="relative text-white text-3xl font-black px-5">Ca sĩ</span>
        </span>
        <div @click="navigateTo('/singer')" class="btn btn-primary btn-outline btn-sm">
          xem thêm
        </div>
      </div>
      <div class="carousel space-x-5">
        <div class="carousel-item" v-for="item in useStore.list" :key="item.id">
          <SingerVMono :data="item" />
        </div>
      </div>
    </div>
  </div>
